...
test("keeps vanilla modules for authentication, XML, templates and storage", async () => {
  ...
  assert.match(auth, /admin@estudiojk\.com\.pe/); assert.match(auth, /register/); assert.match(parser, /ss:Index/); assert.match(parser, /parseSunatXml/); assert.match(templates, /payrollDocument/); assert.match(templates, /feeDocument/); assert.match(templates, /downloadPayrollPdf/); assert.match(templates, /downloadFeePdf/); assert.match(templates, /drawPayrollLogo\(ctx, company, 900, 20, color\)/); assert.match(templates, /\["PERIODO", details\.period\]/); assert.match(templates, /height: 34, size: 16/); assert.match(templates, /Math\.min\(1605, Math\.max\(1575, contributionsEnd \+ 30\)\)/); assert.match(templates, /company\.logoData/); assert.match(templates, /company\.color/); assert.match(templates, /application\/pdf/); assert.match(storage, /localStorage/); assert.match(supabaseSchema, /create table public\.plantillas_generadas/i); assert.match(supabaseSchema, /create table public\.honorarios/i); await access(new URL("../public/hero-office.png", import.meta.url)); assert.match(publicHtml, /app\.js/);
});
