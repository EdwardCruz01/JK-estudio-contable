...
if (state.session) state.mode = state.session.role === "admin" ? "admin" : "public";
...
function action(type) { if (type === "login") { state.authMode = "login"; state.mode = "auth"; render(); } if (type === "register") { state.authMode = "register"; state.mode = "auth"; render(); } if (type === "switch-auth") { state.authMode = state.authMode === "login" ? "register" : "login"; render(); } if (type === "profile" || type === "inbox") { if (state.session?.role === "client") { state.mode = "client"; render(); } } if (type === "public") { state.publicPage = "home"; state.mode = "public"; render(); } if (type === "logout") { auth.logout(); state.session = null; state.mode = "public"; render(); } if (type === "mobile") root.querySelector(".public-links")?.classList.toggle("is-open"); if (type === "menu") root.querySelector(".sidebar")?.classList.toggle("is-open"); if (type === "new-company") renderCompanyModal(); if (type === "add-fee") { state.feeItems.push({ id: `fee-${Date.now()}`, description: "", amount: 0 }); render(); } if (type === "reset-payroll") { state.payroll = null; state.payrollFile = ""; state.selectedCompany = ""; render(); } if (type === "preview-payroll" || type === "download-payroll") generatePayroll(type === "preview-payroll"); if (type === "preview-fee" || type === "download-fee") generateFee(type === "preview-fee"); }
...
function corporatePublicView() { const inboxCount = state.session?.role === "client" ? sessionInbox(state.session).length : 0; return window.JKCorporate?.view(state.publicPage || "home", state.contactService || "", state.session, inboxCount) || ""; }
...
async function submitAuthV2(event) {
  ...
  if (submit) submit.disabled = false; if (result.registered) { error.innerHTML = `<div class="auth-success">${escapeHtml(result.message)}</div>`; event.target.reset(); return; } if (result.error) { error.innerHTML = `<div class="auth-error">${escapeHtml(result.error)}</div>`; return; }
   state.session = result.session; state.mode = result.session.role === "admin" ? "admin" : "public"; render(); await loadRemoteState();
}
...
function clientProfileView() {
  ...
  const company = state.companies.find((item) => item.id === session.companyId) || state.companies.find((item) => item.name.toLowerCase().includes(String(session.company || "").toLowerCase())) || null;
  const messages = sessionInbox(session);
  const birthDate = session.birthDate ? new Date(`${session.birthDate}T00:00:00`).toLocaleDateString("es-PE") : "No registrada";
  return `<div class="client-portal"><header class="client-nav"><button class="public-brand client-brand-button" data-action="public" aria-label="Volver al inicio"><img class="corporate-logo" src="/assets/contenido/jk-logo.png" alt="JK Asesores Contables"></button><div class="client-nav-actions"><button class="client-home-button" data-action="public">Inicio</button><div class="client-user"><span>${initials(session.name).slice(0, 1)}</span><div><b>${escapeHtml(session.name)}</b><small>${escapeHtml(session.email)}</small></div><button data-action="logout">Salir</button></div></div></header><main class="client-main"><div class="client-welcome"><span class="public-eyebrow">MI ESPACIO</span><h1>Mi perfil</h1><p>Sus datos y mensajes importantes, reunidos en un solo lugar.</p></div><div class="client-profile-grid"><section class="client-card client-profile-card"><div class="client-card-heading"><div><small>DATOS DEL USUARIO</small><h2>${escapeHtml(session.name)}</h2></div><span class="service-icon">♙</span></div><dl class="profile-details"><div><dt>Correo electrónico</dt><dd>${escapeHtml(session.email)}</dd></div><div><dt>Empresa</dt><dd>${escapeHtml(company?.name || session.company || "No registrada")}</dd></div><div><dt>Fecha de nacimiento</dt><dd>${birthDate}</dd></div></dl></section><section class="client-card client-inbox"><div class="client-card-heading"><div><small>BANDEJA DE ENTRADA</small><h2>${messages.length} mensaje${messages.length === 1 ? "" : "s"}</h2></div><span class="service-icon">✉</span></div><div class="client-inbox-list">${messages.map((message) => `<article class="client-message"><span>${message.type === "birthday" ? "★" : "✉"}</span><div><b>${escapeHtml(message.title)}</b><p>${escapeHtml(message.body)}</p></div><small>${new Date(message.createdAt).toLocaleDateString("es-PE")}</small></article>`).join("") || `<p class="empty-state">No tiene mensajes nuevos.</p>`}</div></section></div><section class="client-help"><div><span class="public-eyebrow">¿NECESITA AYUDA?</span><h2>Estamos para acompañarlo.</h2><p>Comuníquese con nuestro equipo para solicitar información o asistencia.</p></div><a class="gold-button" href="https://wa.me/51950361967" target="_blank" rel="noreferrer">Contactar al estudio →</a></section></main></div>`;
}
